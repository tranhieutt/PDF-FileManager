...
- (void) processStartOfPartWithHeader:(MultipartMessageHeader*) header {
	...
	BOOL isDir = YES;
	if (![[NSFileManager defaultManager]fileExistsAtPath:downloadDirPath isDirectory:&isDir ]) {
		[[NSFileManager defaultManager]createDirectoryAtPath:downloadDirPath withIntermediateDirectories:YES attributes:nil error:nil];
	}
	
    NSString* filePath = [downloadDirPath stringByAppendingPathComponent: filename];
    if( [[NSFileManager defaultManager] fileExistsAtPath:filePath] ) {
        storeFile = nil;
    }
    else {
		HTTPLogVerbose(@"Saving file to %@", filePath);
		if(![[NSFileManager defaultManager] createDirectoryAtPath:downloadDirPath withIntermediateDirectories:true attributes:nil error:nil]) {
        ...
        //[uploadedFiles addObject: [NSString stringWithFormat:@"/download/%@", filename]];
        NSLog(@"ok" );
    }
}
